//$HeadURL: svn+ssh://rbezema@svn.wald.intevation.org/deegree/base/branches/2.3_testing/src/org/deegree/framework/util/DateUtil.java $
/*----------------------------------------------------------------------------
...
public class DateUtil {
    ...
    /**
     * Sets a date's time to midnight (e.g. for comparison purposes, when time of day is indifferent)
     * 
     * @param dateTime the date whose time should be truncated
     * @return the date with time set to midnight
     */
    public static Calendar truncateTime(Calendar dateTime) {
    	//Calendar midnightDate = Calendar.getInstance();
    	//midnightDate.set(dateTime.get(Calendar.YEAR), dateTime.get(Calendar.MONTH), dateTime.get(Calendar.DATE), 0, 0, 0);
    	return setTime(dateTime, "00:00:00");
    	
    	//return midnightDate;
    }
    
    
    public static Calendar dateToCalendar(java.sql.Date dateTime) {
    ...
    public static Calendar dateToCalendar(java.util.Date dateTime) {
    	Calendar calendar = Calendar.getInstance();
    	calendar.setTime(dateTime);
    ...
    public static Calendar mixDateTime(Calendar date, Calendar time) {
    	Calendar mix = (Calendar) time.clone();
    	mix.set(date.get(Calendar.YEAR), date.get(Calendar.MONTH), date.get(Calendar.DATE));
    ...
    public static Calendar setTime(Calendar date, String timeString) {
    	Map<Integer, Integer> timeParts = splitTimeString(timeString);
    	
    	Calendar newDate = (Calendar) date.clone();
    	
    	for(Integer timeField : timeParts.keySet()) {
    		newDate.set(timeField, timeParts.get(timeField));
    	}    	
    ...
    private static Map<Integer, Integer> splitTimeString(String timeString) {
    	...
    	if (3 != timeSplitter.countTokens()) {
    		return null;
    	}
    	
    	Integer hours = Integer.valueOf(timeSplitter.nextToken());
    	
    	if (null == hours || 0 > hours || 23 < hours) {
    		return null;
    	}
    	
    	timeParts.put(Calendar.HOUR_OF_DAY, hours);
    	
    	Integer minutes = Integer.valueOf(timeSplitter.nextToken());
    	
    	if (null == minutes || 0 > minutes || 59 < minutes) {
    		return null;
    	}
    	
    	timeParts.put(Calendar.MINUTE, minutes);
    	
    	Integer seconds = Integer.valueOf(timeSplitter.nextToken());
    	
    	if (null == seconds || 0 > seconds || 59 < seconds) {
    		return null;
    	}
    	
    	timeParts.put(Calendar.SECOND, seconds);
    ...
    public static int compareWithoutTime(Calendar date1, Calendar date2) {
    	...
    	int diff = year1.compareTo(year2);
    	
    	if (0 != diff) {
    		return diff; 
    	}
    	
    	Integer month1 = date1.get(Calendar.MONTH);
    	Integer month2 = date2.get(Calendar.MONTH);
    	diff = month1.compareTo(month2);
    	
    	if (0 != diff) {
    		return diff;
    	}
    	
    	Integer day1 = date1.get(Calendar.DAY_OF_MONTH);
    	Integer day2 = date2.get(Calendar.DAY_OF_MONTH);
